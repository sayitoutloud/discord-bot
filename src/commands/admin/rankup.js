...
export default {
  data: new SlashCommandBuilder()
    .setName('rankup')
    .setDescription('Erhöht den Rank eines Members (nur Rank Manager).')
  ...
  async execute(interaction) {
    ...
    // Berechtigung: Rank Manager oder ManageRoles
    ...
    if (!current) {
      // kein grade -> gib die letzte (max number) as start (niedrigste)
      targetGrade = grades[grades.length - 1];
    } else {
      // next higher = number - 1 (1 = highest)
      targetGrade = GradeSystem.getHigherGrade(current);
      if (!targetGrade) {
        return interaction.reply({ content: `ℹ️ ${member.user.tag} ist bereits auf der höchsten Grade.`, ephemeral: true });
      }
    }
